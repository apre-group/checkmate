...
void Node::add_violation_cr() const {
	if(this->is_leaf() || this->is_branch()) {
		size_t limit;
		this->is_leaf() ? limit = this->leaf().conditions.size() : limit = this->branch().conditions.size();
...
bool equivalent_conditions(z3::Bool c1, z3::Bool c2) {
	// c1 <=> c2 iff c1=>c2 && c2=>c1
	// that is iff (!c1 \lor c2) and (!c2 \lor c1) valid
	// that is iff (c1 \and !c2) lor (c2 \land !c1) unsat

	auto conj1 = z3::conjunction({c1, !c2});
	auto conj2 = z3::conjunction({c2, !c1});
	auto disj = z3::disjunction({conj1, conj2});

	z3::Solver solver;
	if (solver.solve({disj}) == z3::Result::UNSAT) {
		return true;
	}
	return false;
...
bool implies(z3::Bool c1, z3::Bool c2) {
	// c1=>c2 valid iff !c1 \lor c2 valid iff c1 && !c2 unsat
	// that is iff (!c1 \lor c2) and (!c2 \lor c1) valid
	// that is iff (c1 \and !c2) lor (c2 \land !c1) unsat

	auto conj = z3::conjunction({c1, !c2});

	z3::Solver solver;
	if (solver.solve({conj}) == z3::Result::UNSAT) {
		return true;
	}
	return false;
...
bool duplicate_exists(HistoryChoice &entry, std::vector<HistoryChoice> &choices) {
	...
	for(auto &choice : choices) {
		if(entry.history == choice.history && equivalent_conditions(entry.condition, choice.condition)) {
			return true;
		}
	}
	return false;
...
std::vector<HistoryChoice> Node::compute_pr_strategy(std::vector<std::string> players, std::vector<std::string> actions_so_far, std::vector<std::string>& strategy_vector, std::vector<z3::Bool>& strategy_condition, z3::Bool condition_cu) const {
	...
	// if you can find strategy_condition[0] -> great!
	// work with all such conditions
	// else choose condition c from branch.conditions such that
	// cu.condition (function paramenter) -> c

	bool found = false;
	for (auto &condition : this->branch().conditions) {
		if(equivalent_conditions(condition.condition, strategy_condition[0])) {
			found = true;

			// go through all the actions

			// if you find strategy_vector[0]
			// set player -> current player
			// 		history -> actions so far
			// 		condition -> c
			//		choice -> choice
			// pop strategy[0] and condition [0]
			// actions so far add choice action 
			// and go to choice node

			for(auto &choice: condition.children) {
				if (choice.action == strategy_vector[0]) {

					HistoryChoice hist_choice;
					hist_choice.player = players[this->branch().player];
					hist_choice.choice = strategy_vector[0];
					hist_choice.history = actions_so_far;
					hist_choice.condition = condition.condition;
					
					strategy.push_back(hist_choice);

					strategy_vector.erase(strategy_vector.begin());
					strategy_condition.erase(strategy_condition.begin());

				}
				
				// update actions so far wiht choice action 
				// and go to choice node

				std::vector<std::string> updated_actions;
				updated_actions.insert(updated_actions.end(), actions_so_far.begin(), actions_so_far.end());
				updated_actions.push_back(choice.action);

				auto res = choice.node->compute_pr_strategy(players, updated_actions, strategy_vector, strategy_condition, condition_cu);
				strategy.insert(strategy.end(), res.begin(), res.end());
	...
	if(!found) {
		for (auto &condition : this->branch().conditions) {
			...
			if(implies(condition_cu, condition.condition)) {

				// same code as above, can be refactored as a function
...
CeCase Node::compute_pr_cecase(std::vector<std::string> players, unsigned current_player, std::vector<std::string> actions_so_far, std::vector<z3::Bool> conditions_so_far, std::string current_action, z3::Bool current_condition, ConditionalUtilities practical_utilities, size_t index) const {
	...
	if(current_player < players.size()) {
		...
		std::vector<z3::Bool> conditions_to_deviation;
		conditions_to_deviation.insert(conditions_to_deviation.end(), conditions_so_far.begin(), conditions_so_far.end());
...
const Node* Node::compute_deviation_node(std::vector<std::string> actions_so_far, std::vector<z3::Bool> conditions_so_far) const {

	if(actions_so_far.size() > 0) {
		assert(!this->is_leaf());
		assert(!this->is_subtree());
		for(auto &condition : this->branch().conditions) {
			if(equivalent_conditions(condition.condition, conditions_so_far[0]))
			{
				for (const auto &child: condition.children){
					if(child.action == actions_so_far[0]) {
						actions_so_far.erase(actions_so_far.begin());
						conditions_so_far.erase(conditions_so_far.begin());
						return child.node->compute_deviation_node(actions_so_far, conditions_so_far);
					}
				}
			}
		}
...
std::vector<CeChoice> Node::compute_pr_ce(std::string current_action, z3::Bool current_condition, std::vector<std::string> actions_so_far, std::vector<z3::Bool> conditions_so_far, ConditionalUtilities practical_utilities, size_t index) const {
	...
	for (size_t i = 0; i < practical_utilities.condition.size(); i++) {
		...
		for(auto &utility : practical_utilities_set) {
			...
			updated_conditions.insert(updated_conditions.end(), conditions_so_far.begin(), conditions_so_far.end());
			updated_conditions.push_back(current_condition);
			cechoice.history_conditions = updated_conditions;
...
void Node::strat2hist(std::vector<std::string> &strategy_copy, std::vector<z3::Bool> &conditions_copy, z3::Bool &condition, UtilityTuple utility, std::vector<std::string> &pruned_history, std::vector<z3::Bool> &pruned_conditions, bool &stop_reached, bool &first_condition) const {
	...
	//		remove first elements of strategy_copy and conditions_copy
	//		for node after each action call prune_actions
	// if condition implies c
	// 		go through every action a
	// 			if a is NOT the first element of strategy copy
	//				for node after a call prune_actions
...
void Node::prune_actions_from_strategy(std::vector<std::string> &strategy, std::vector<z3::Bool> &conditions) const {
	...
	for (auto &condition : this->branch().conditions) {
		...
		for(auto &child: condition.children) {
			child.node->prune_actions_from_strategy(strategy, conditions);
		}
	}
}
