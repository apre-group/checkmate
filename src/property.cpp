	...
	if (node->is_leaf()) {
		...
		if (!group[branch.player]) { 
			...
			if (branch.honest) {
				...
				if (collusion_resilience_rec(input, solver, options, subtree, group, all_honest_total, players, group_nr, consider_prob_groups)) {
					if (consider_prob_groups) {
...
bool practicality_rec(const Input &input, const Options &options, z3::Solver &solver, Node *node, std::vector<std::string> actions_so_far, bool consider_prob_groups) {
	...
	if (node->is_leaf()) {
	...
	} else if (node->is_condition_node()) { // TO BE EDITED
		const auto &cond_node = node->condition_node();
		...
		if (branch.honest) {
			...
			for (const auto& hon_utility: honest_utilities){
				...
				UtilityTuple cleared_strategy(hon_utility.leaf, hon_utility.condition); 
				to_clear_strategy.insert(cleared_strategy);
			}


			for ( auto& honest_utility: to_clear_strategy) {
			...
			branch.practical_utilities = to_clear_strategy;
			...
			UtilityTuplesSet utility_result; // KIM conditions
			unsigned int k = 0;
			for (const auto& utilities : children) {
				for (const auto& utility : utilities) {
					UtilityTuple to_insert(utility.leaf); // TODO consider condition
			...
			// the set to drop
			UtilityTuplesSet remove; // KIM conditions
	...
	if (property == PropertyType::WeakImmunity || property == PropertyType::WeakerImmunity) {
		...
		for (uint64_t binary_counter = 1; binary_counter < -1ull >> (64 - input.players.size()); binary_counter++) {
			...
			if (!input.solved_for_group[binary_counter]){
				...
				// problematic groups are only considered when we haven't found a case split point yet
				bool collusion_resilient_for_group = collusion_resilience_rec(input, solver, options, input.root.get(), group, all_honest_total, input.players.size(), binary_counter, true);
...
bool property_rec_subtree(z3::Solver &solver, const Options &options, const Input &input, const PropertyType property, std::vector<z3::Bool> current_case, size_t history, unsigned group_nr, std::vector<std::vector<z3::Bool>> &satisfied_in_case) {
	...
	*/
	...
	if(property == PropertyType::CollusionResilience){
		...
		property_result = collusion_resilience_rec(input, solver, options, input.root.get(), group, all_honest_total, input.players.size(), group_nr, false);
	} else {
...
void property_subtree_utility(const Options &options, const Input &input, PropertyType property, HonestUtilityTuple honest_utility, Subtree &subtree) {
	...
	std::vector<CondHonestUtility> honest_utility_vec = get_conditional_honest_utilities(honest_utility.element);

	std::cout << std::endl;
	std::cout << std::endl;
	std::cout << "Is utility " << honest_utility_vec << " collusion resilient?" << std::endl;
