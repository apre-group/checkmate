...
intermediate_state[ps[0]] = {}
for player in ps:
    i = ps.index(player)
    if i < len(ps) - 1:
        intermediate_state[ps[i+1]] = {}
    if i == 0:
        intermediate_state[player]["contract"] = "null"
        intermediate_state[player]["amount_to_unlock"] = None
        intermediate_state[ps[i+1]]["amount_to_unlock"] = m + (len(ps) - 2 - i) * f
    else:
        intermediate_state[player]["contract"] = "locked"
        if i < len(ps) -1:
            intermediate_state[ps[i+1]]["amount_to_unlock"] = m + (len(ps) - 2 - i) * f
    intermediate_state[player]["secrets"] = {p: False for p in ps}
    intermediate_state[player]["ignoreshare"] = {p: False for p in ps}
intermediate_state[ps[-1]]["secrets"][ps[-1]] = True
align_secret_knowledge(intermediate_state)

unlocking_tree = generate_routing_unlocking(ps[-1], intermediate_state, False, "")
